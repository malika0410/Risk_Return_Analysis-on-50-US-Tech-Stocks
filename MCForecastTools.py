# This script was supplied by the 
# UC Berkeley Extension FinTech program
# slightly modified to work for our project
...
class MCSimulation:
    ...
    def __init__(self, portfolio_data, weights="", num_simulation=1000, num_trading_days=252):
        ...
        if not "daily_return" in portfolio_data.columns.get_level_values(1).unique(): 
            ...
            portfolio_data = portfolio_data.merge(close_df,left_index=True,right_index=True).reindex(columns=tickers,level=1)    
    ...
    def calc_cumulative_return(self):
        ...
        last_prices = self.portfolio_data.xs('Close',level=0,axis=1)[-1:].values.tolist()[0]
        
        # Calculate the mean and standard deviation of daily returns for each stock
        daily_returns = self.portfolio_data.xs('daily_return',level=0,axis=1)
